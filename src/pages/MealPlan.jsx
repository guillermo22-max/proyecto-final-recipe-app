        ...
        <table className="table border-rounded">
          <thead>
            <tr>
              ...
              {daysOfWeek.map((day) => (
                <th key={day} className="text-center bg-success">{day}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {meals.map((meal) => (
              <tr key={meal}>
                <td className="day-cell bg-success"><strong className="day-text">{meal}</strong></td>
                {daysOfWeek.map((day) => (
                  <td
                    key={`${day}-${meal}`}
                    onDrop={(e) => {
                    ...
                    className="meal-cell bg-success bg-opacity-25" 
                  >
                    {mealPlan[`${day}-${meal}`] ? (
                      <div
